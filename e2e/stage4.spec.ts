...
test("mobile sequential navigation, search and Back remain usable", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 }); await login(page); const firstNotebook = page.locator("[data-notebook-title]").first(); await firstNotebook.getByRole("button").filter({ hasText: /.+/ }).first().click(); const section = page.locator("aside").getByRole("button").filter({ hasText: /Серверы|Перенос|Section/ }).first(); await section.click(); await page.getByRole("button", { name: "Новая страница" }).click(); await expect(page.getByPlaceholder("Название страницы")).toBeVisible(); await page.getByRole("button", { name: "Поиск" }).click(); await expect(page.getByRole("dialog", { name: "Быстрый переход" })).toBeVisible(); await page.keyboard.press("Escape"); await page.getByRole("button", { name: "Страницы" }).click(); await expect(page.getByRole("button", { name: "Новая страница" })).toBeVisible();
});
