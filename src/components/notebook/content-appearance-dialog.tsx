...
export function PageAppearanceDialog({ page, onClose, onSaved }: { page: PageSummary; onClose(): void; onSaved(page: PageSummary): void }) {
  ...
  useEffect(() => { void api<{ settings: { editorContentWidth: typeof width } }>("/api/account/preferences").then(({ settings }) => setWidth(settings.editorContentWidth)); }, []);
  ...
  async function save() { setBusy(true); try { const [{ page: saved }] = await Promise.all([api<{ page: PageSummary }>(`/api/pages/${page.id}`, jsonOptions("PATCH", { icon: icon || null, color, coverUploadId })), api("/api/account/preferences", jsonOptions("PATCH", { editorContentWidth: width }))]); onSaved(saved); onClose(); } finally { setBusy(false); } }
