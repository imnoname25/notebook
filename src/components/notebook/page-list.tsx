...
export function PageList(props: Props) {
  ...
  return <aside className="flex h-full min-h-0 flex-col bg-background p-3 md:border-r md:border-border/60">
    <div className="mb-2 flex min-h-9 items-center gap-2 px-1">
      ...
      <div className="min-w-0 flex-1"><p className="truncate text-[15px] font-semibold">{props.section?.title ?? "Страницы"}</p><p className="text-[12.5px] text-muted-foreground">{ordered.length} стр.</p></div>
    ...
    <div className="min-h-0 flex-1 space-y-1 overflow-y-auto">
      ...
      {props.loading && <div className="space-y-1 p-1"><div className="h-14 animate-pulse rounded-lg bg-muted"/><div className="h-14 animate-pulse rounded-lg bg-muted"/></div>}
      ...
      {!props.loading && <SortableList ids={ordered.map((page) => page.id)} onReorder={props.onReorder}>{ordered.map((page) => <SortableItem id={page.id} key={page.id}>{({ setNodeRef, style, attributes, listeners, isDragging, isOver }) => <div ref={setNodeRef} style={style} data-page-title={page.title} className={cn("group flex rounded-lg", props.activePageId === page.id ? "bg-accent text-accent-foreground" : "hover:bg-accent/60", isDragging && "opacity-60 shadow-md", isOver && !isDragging && "ring-1 ring-primary/40")}>
        ...
        <button className="min-w-0 flex-1 px-2 py-2 text-left" onClick={() => props.onSelect(page)}><div className="flex items-center gap-1.5"><p className="truncate text-[14.5px] font-medium">{page.title || "Без названия"}</p>{page.isFavorite && <Star size={12} className="shrink-0 fill-current text-amber-500" />}</div><time className="mt-0.5 block text-[12.5px] text-muted-foreground">{new Intl.DateTimeFormat("ru", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(page.updatedAt))}</time></button>
