...
export function LoginForm({ needsSetup }: { needsSetup: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(needsSetup ? "/api/auth/setup" : "/api/auth/login", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
  ...
  return <main className="flex min-h-dvh items-center justify-center bg-muted/40 px-5 py-12">
    <section className="w-full max-w-sm rounded-2xl bg-card p-7 shadow-sm ring-1 ring-border/60">
      ...
      <h2 className="mb-1 text-lg font-medium">{needsSetup ? "Создание администратора" : "С возвращением"}</h2>
      <p className="mb-6 text-sm text-muted-foreground">{needsSetup ? "Это первый запуск. Создайте локальную учётную запись." : "Войдите в свою записную книжку."}</p>
      <form className="space-y-4" onSubmit={submit}>
        {needsSetup && <label className="block text-sm font-medium">Имя<Input className="mt-1.5" name="name" required autoComplete="name" /></label>}
        ...
        <label className="block text-sm font-medium">Пароль<Input className="mt-1.5" name="password" type="password" minLength={8} required autoComplete={needsSetup ? "new-password" : "current-password"} /></label>
        {error && <p className="rounded-lg bg-destructive/10 px-3 py-2 text-sm text-destructive" role="alert">{error}</p>}
        <Button className="w-full" disabled={loading}>{loading && <Loader2 className="animate-spin" size={16} />}{needsSetup ? "Создать и войти" : "Войти"}</Button>
      </form>
    </section>
  </main>;
}
