...
import { ApiError, apiError, readJson, requireAdmin, validateRequestOrigin } from "@/lib/api";
...
export async function POST(request: NextRequest) {
  try {
    validateRequestOrigin(request); await requireAdmin(); const input = settingsUpdateSchema.pick({ s3Endpoint: true, s3Region: true, s3Bucket: true, s3AccessKeyId: true, s3SecretAccessKey: true, s3Prefix: true, s3ForcePathStyle: true }).parse(await readJson(request)); const stored = await getApplicationSettings();
