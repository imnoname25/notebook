...
export const sectionUpdateSchema = z.object({ title, icon: z.string().max(40).nullable(), parentId: z.string().min(1).nullable() }).partial().strict();
...
export const pageUpdateSchema = z.object({ title, content: z.array(z.record(z.string(), z.unknown())), isFavorite: z.boolean(), expectedRevision: z.number().int().min(0), snapshotReason: z.enum(["interval", "manual"]) }).partial().strict();
