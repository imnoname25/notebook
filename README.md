...
`Настройки → Профиль` позволяет изменить имя и email, а `Настройки → Безопасность` — проверить текущий пароль и установить новый. Смена пароля завершает все сеансы, включая текущий, после чего требуется войти снова. Вставка из password manager разрешена; поля используют `current-password` и `new-password` autocomplete.

Роль `ADMIN` открывает `Настройки → Пользователи`: создание локальных пользователей, изменение имени/email/роли, временный пароль с обязательной сменой при входе, блокировка, разблокировка, сброс пароля и 2FA, а также отзыв всех сеансов. Публичной регистрации после первого setup нет. Обычный пользователь видит только профиль, безопасность и собственные данные Notebook.

Первый setup-пользователь всегда получает роль `ADMIN`; новые записи по умолчанию получают `USER`. Сервер транзакционно запрещает блокировку и понижение роли последнего активного администратора, а self-disable через UI запрещён. Блокировка не удаляет Notebook/Section/Page/uploads. Постоянное удаление пользователя намеренно отсутствует, пока filesystem uploads нельзя гарантированно удалить одной согласованной операцией.

Персональные density/editor preferences хранятся в `UserSettings`. Настройки backup, storage, remote providers, системная диагностика и operational notifications являются application settings и доступны только администраторам.
